...
make_fixture(kfd::Device &dev, kfd::Dim3 grid = {.x = 1},
             kfd::Dim3 block = {.x = 64}) {
    ...
    DYNAMIC_SECTION("device " << di) {
      ...
      auto worker = [&](unsigned tid) {
        for (unsigned i = 0; i < ITERS; ++i) {
          ...
          REQUIRE_RESULT(signals[tid].wait(kfd::Condition::EQ, 0,
...
TEST_CASE("Signal - wait_any returns first completed signal",
  ...
  for (size_t di = 0; di < ctx.num_devices(); ++di) {
    DYNAMIC_SECTION("device " << di) {
      ...
      auto result = kfd::wait_any(ptrs, kfd::Condition::EQ, 0,
                                  kfd::test::WAIT_TIMEOUT_NS);
      ...
      for (unsigned i = 0; i < N; ++i) {
        if (i == TARGET)
          continue;
        uint64_t val =
            __atomic_load_n(signals[i].fence_addr(), __ATOMIC_ACQUIRE);
        CHECK(val != 0);
      }
    }
  }
...
TEST_CASE("Signal - wait_all blocks until all signals complete",
  ...
  for (size_t di = 0; di < ctx.num_devices(); ++di) {
    DYNAMIC_SECTION("device " << di) {
      ...
      REQUIRE_RESULT(kfd::wait_all(ptrs, kfd::Condition::EQ, 0,
                                   kfd::test::WAIT_TIMEOUT_NS));
