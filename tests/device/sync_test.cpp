...
TEST_CASE("Sync - SDMA fill then compute dispatch", "[device][sync]") {
  ...
  auto fix = make_device_fixture(ctx, sync_kernels);
  REQUIRE_RESULT(fix);
  ...
  REQUIRE_RESULT(done->wait(kfd::Condition::EQ, 0, kfd::test::WAIT_TIMEOUT_NS));
...
TEST_CASE("Sync - compute dispatch then SDMA readback", "[device][sync]") {
  ...
  REQUIRE_RESULT(fix->sdma.copy_linear(readback.data(), out.data(), BUF_BYTES));
