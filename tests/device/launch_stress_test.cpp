...
std::expected<LaunchFixture, kfd::Error> make_fixture(kfd::Context &ctx) {
...
TEST_CASE("Launch - block size sweep", "[device][launch]") {
  ...
  static constexpr uint32_t sizes[] = {1, 32, 33, 64, 65, 128, 256, 512, 1024};
...
TEST_CASE("Launch - two independent compute queues", "[device][launch]") {
  ...
  REQUIRE_RESULT(sig1->wait(kfd::Condition::EQ, 0, kfd::test::WAIT_TIMEOUT_NS));
  REQUIRE_RESULT(sig2->wait(kfd::Condition::EQ, 0, kfd::test::WAIT_TIMEOUT_NS));
}
