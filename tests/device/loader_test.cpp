...
TEST_CASE("Loader - load succeeds for compatible arch", "[device][loader]") {
  ...
  auto fix = make_device_fixture(ctx, device_kernels);
  REQUIRE_RESULT(fix);

  INFO("arch: " << fix->bin->arch);
  auto buf = read_file(fix->bin->path);
  auto loaded = kfd::Executable::load(*fix->gpu, buf, fix->sdma, fix->compute);
...
TEST_CASE("Loader - global values readable via SDMA", "[device][loader]") {
  ...
  for (const auto &[name, expected] : globals) {
    ...
    REQUIRE_RESULT(
        fix->sdma.copy_linear(readback->data(), sym->data(), sizeof(unsigned)));
