...
TEST_CASE("PM4 - COND_INDIRECT_BUFFER runs the IB only when the compare holds",
          "[device][pm4]") {
  auto &ctx = require_ctx();
  for (size_t di = 0; di < ctx.num_devices(); ++di) {
    DYNAMIC_SECTION("device " << di) {
      ...
      auto ctrl = alloc_host_buffer(gpu);
      auto ibbuf = alloc_host_buffer(gpu);
      auto leafbuf = alloc_host_buffer(gpu);
      auto *c = static_cast<volatile uint32_t *>(ctrl.data());
      auto *ib = static_cast<uint32_t *>(ibbuf.data());
      auto *leaf = static_cast<uint32_t *>(leafbuf.data());

      constexpr uint32_t RUN_MAGIC = 0x11111111u;
      uint32_t ln =
          pm4::write_data(leaf, const_cast<uint32_t *>(&c[TARGET]), RUN_MAGIC);

      // Run the IB only when (gate >= 3); otherwise fall through unchanged.
      uint32_t n = pm4::cond_indirect_buffer(
          ib, kfd::Condition::GTE, const_cast<uint32_t *>(&c[GATE]),
          /*mask=*/0xFFFFFFFFull, /*reference=*/3ull, leaf, ln);
      ...
      INFO("gate = 5 (>= 3) runs the conditional IB");
      REQUIRE(run(5));
      CHECK(c[TARGET] == RUN_MAGIC);

      INFO("gate = 1 (< 3) falls through, leaving the target untouched");
      REQUIRE(run(1));
      CHECK(c[TARGET] == SENTINEL);
    }
  }
